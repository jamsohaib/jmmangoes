...
const WarehouseManagement = () => {
  ...
  const canView = user?.role === 'admin' || user?.permissions?.warehouseManagement?.view;
  const canManage = user?.role === 'admin' || user?.permissions?.warehouseManagement?.manage;
  ...
  useEffect(() => { if (canView) load().catch(() => toast.error('Failed to load warehouses')); }, [canView]);
  ...
  return (
    <div className="p-3 md:p-4 text-black">
      ...
      <form onSubmit={submit} className="bg-white rounded shadow p-4 grid grid-cols-1 md:grid-cols-3 gap-2 mb-4">
        <input className="border p-2 rounded" placeholder="Warehouse Name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required />
        <input className="border p-2 rounded" placeholder="Warehouse Code" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} required />
